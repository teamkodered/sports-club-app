...
export default function StudentProfile({ student, onClose, isAdmin }) {
  ...
  async function saveEdit() {
    setSaving(true)
    const { house_id, ...studentFields } = editForm
    await supabase.from('students').update(studentFields).eq('id', localStudent.id)
            ...
            <div>
              {!editing ? (
                <>
                  ...
                  {isAdmin && <button className="btn btn-sm" style={{ marginTop: 14 }} onClick={() => { setEditForm({ pka_belt: localStudent.pka_belt, krba_level: localStudent.krba_level, age_category: localStudent.age_category, competition_team: localStudent.competition_team, weight_kg: localStudent.weight_kg, weight_category: localStudent.weight_category, media_restriction: localStudent.media_restriction, media_notes: localStudent.media_notes, medical_conditions: localStudent.medical_conditions, medication: localStudent.medication, house_id: localStudent.members?.house_id || '', is_kr: localStudent.is_kr || false, is_pts: localStudent.is_pts || false, is_leader: localStudent.is_leader || false, is_coach: localStudent.is_coach || false }); setEditing(true) }}>Edit record</button>}
                </>
              ) : (
                <div>
