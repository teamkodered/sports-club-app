...
export default function AthleteProfiles() {
  ...
  async function saveFit2FightSession() {
    ...
    // VO2 Max still lives in the same test:{} map as everything else that
    // used to be under the old Test tab -- merge it in rather than
    // overwrite, so any other test values already on this row survive.
    const existingRow = f2fData.find(s => s.id === sessionForm.id)
    const mergedTest = { ...(existingRow?.test || {}) }
    if (sessionForm.vo2max === '' || sessionForm.vo2max == null) delete mergedTest['VO2 Max']
    else mergedTest['VO2 Max'] = sessionForm.vo2max
    const payload = {
      student_id: selected.id,
      ...
      test: Object.keys(mergedTest).length ? mergedTest : null,
    }
    let error
    if (sessionForm.id) {
                ...
                <div className="card" style={{ maxWidth: 320, textAlign: 'center' }} onClick={e => e.stopPropagation()}>
                  ...
                  <div ref={profileHeaderRef} className="card" style={{ padding: 0, marginBottom: 14 }}>
                    ...
                    {/* Website Profile preview -- same show_on_website
                        ...
                        what it will show once it is. Lives inside the
                        Profile section's own collapse state (like
                        everything else above) rather than its own
                        always-visible block -- this has been asked for
                        several times now. */}
                  ...
                  <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.35)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50, padding: 16 }}>
                    <div className="card" style={{ width: '100%', maxWidth: 400 }}>
                      ...
                      <div className="field">
                        ...
                        <input type="number" step="0.1" value={sessionForm.vo2max ?? ''} onChange={e => setSessionForm(f => ({ ...f, vo2max: e.target.value }))} />
                      </div>
                      <div className="field">
                        <label>Notes</label>
