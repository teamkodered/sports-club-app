...
// Shared by the Sessions/Results/PDP cards at the top of Home on both
// the athlete and coach views, so the two stay identical in feel.
...
// Matches each PDP section's header text colour -- used for progress
// bars and question/selection highlights within that section, instead
// of everything sharing one single (house-based) accent colour
...
export default function AthleteApp() {
  ...
  // node for each technique's note field, so the Save button can read
  // its current value without needing a separate piece of state per
  // technique (there can be dozens across all styles/categories).
  const techniqueNoteRefs = useRef({})
  const sncDescRefs = useRef({})
  const sprintTestOtherRef = useRef(null)
  const wattIntervalOtherRef = useRef(null)
  const tacticalNoteRefs = useRef({})
            ...
            <>
              ...
              {(() => {
               try {
                ...
                const sessionsPct = possibleSessions ? Math.round((attendedDayCount / possibleSessions) * 100) : 0
                const sessionsValue = `${attendedDayCount}/${possibleSessions || attendedDayCount}`

                // Results (F2F) card values
                ...
                const f2fHasPct = f2fTargetTotal > 0
                const f2fValue = f2fHasPct ? `${f2fCount}/${f2fTargetTotal}` : f2fCount
                const f2fPct = f2fHasPct ? `${Math.round((f2fCount / f2fTargetTotal) * 100)}%` : f2fValue
                ...
                const pdpHasPct = totalSent > 0
                const pdpValue = pdpHasPct ? `${completedCount}/${totalSent}` : completedCount
                const pdpPct = pdpHasPct ? `${Math.round((completedCount / totalSent) * 100)}%` : pdpValue
                        ...
                        <div className="card" style={{ marginBottom: 8 }}>
                          ...
                          <div className="field"><label>{isTimedSprints ? (sprintMode === 'time' ? 'Fixed time' : 'Fixed distance') : 'Specific test'}</label>
                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, alignItems: 'center' }}>
                              ...
                              <button type="button" className="btn btn-sm" style={{ fontSize: 11 }}
                                onClick={() => { const val = sprintTestOtherRef.current?.value; if (val) upsert({ ...entry, test: val }) }}>Save</button>
                            </div>
                            {cat?.hasOnOffInput && (
                              <div style={{ marginTop: 8 }}>
                        ...
                        <div className="card" style={{ marginBottom: 8 }}>
                          ...
                          <div className="field"><label>Interval</label>
                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, alignItems: 'center' }}>
                              ...
                              <input ref={wattIntervalOtherRef} defaultValue={presets.includes(normalizeIntervalMode(entry.interval_mode)) ? '' : (entry.interval_mode || '')}
                                placeholder="Other…" style={{ width: 90, flexShrink: 0 }} />
                              <button type="button" className="btn btn-sm" style={{ fontSize: 11 }}
                                onClick={() => { const val = wattIntervalOtherRef.current?.value; if (val) upsert({ ...entry, interval_mode: val }) }}>Save</button>
                            </div>
                            <div style={{ marginTop: 8 }}>
                        ...
                        {todaysSnc.map((entry, i) => (
                          <div key={i} style={{ marginBottom: 12, paddingBottom: 12, borderBottom: '1px solid var(--border)' }}>
                            ...
                            <div style={{ display: 'flex', gap: 4, marginBottom: 6 }}>
                              <input ref={el => { sncDescRefs.current[i] = el }} defaultValue={entry.description || ''}
                                placeholder="What does this routine consist of?" style={{ flex: 1 }} />
                              <button type="button" className="btn btn-sm" style={{ fontSize: 11 }}
                                onClick={() => { const next = [...todaysSnc]; next[i] = { ...next[i], description: sncDescRefs.current[i]?.value ?? '' }; savePhysicalField('snc', next, setTodaysSnc) }}>Save</button>
                            </div>
              ...
              <div className="card" style={{ borderLeft: '3px solid #E24B4A' }}>
                ...
                {[
                  ...
                  <div key={f.key} className="field" style={{ marginBottom: 10 }}>
                    <label>{f.label}</label>
                    <SavableField rows={2} defaultValue={apData?.pdp_notes?.ready?.[f.key]} placeholder={f.placeholder}
                      onSave={val => saveAthletePdpReadyField(f.key, val)}
                      inputStyle={{ padding: '8px 10px', border: '1px solid var(--border-strong)', borderRadius: 'var(--radius)', fontSize: 13, background: 'var(--bg-secondary)', color: 'var(--text)', fontFamily: 'var(--font-sans)' }} />
                  </div>
                ))}
              </div>
            </div>
                          ...
                          return (
                            <div key={i} id={`my-f2f-entry-${s.id}`} className="card" style={{
                              ...
                              {s.watt_bike && toEntries(s.watt_bike).map((e, ei) => (
                                <p key={ei} style={{ fontSize: 12, margin: '4px 0' }}>
                                  🚴 {formatIntervalLabel(normalizeIntervalMode(e.interval_mode || e.type)) || 'Watt bike'}{(e.sets?.length) ? `: ${e.sets.map(v => (v && typeof v === 'object') ? v.wattage : v).join(', ')}W` : ' logged'}
                                </p>
                              ))}
