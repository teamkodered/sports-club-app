                ...
                <tr key={s.id} style={isPending ? { background: '#fef9e744', borderLeft: '3px solid #EF9F27' } : {}}>
                  {cols.map(c => {
                    switch(c.key) {
                      ...
                      case 'joined_date': return <td key={c.key} style={{ fontSize: 11, color: 'var(--text-secondary)' }}>{m?.joined_date || '—'}</td>
                  ...
                  <td>
                    <button className="btn btn-sm" onClick={() => setSelected(s)}>View</button>
                  </td>
                </tr>
              )
            })}
